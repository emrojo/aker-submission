...
class SubmissionsController < ApplicationController
  ...
  def show
    skip_step if step == :ethics && !any_human_material_no_hmdmc?
    render_wizard
  end
  ...
  def update
    ...
    return ethics_update if params[:id] == 'ethics'
    ...
    if params[:id] == 'provenance'
      ...
      @status_success = manifest.update(manifest_params) if @status_success
    else
    ...
    if last_step?
      ...
      begin
        ...
        @manifest.broken! if !success && !cleanup
      end
  ...
  def get_next_status
    ...
    last_step? ? Manifest.ACTIVE : next_step.to_s
  end
