...
module Printables::Group
  def print_tubes(printer_name, label_template_name, printables)
    ...
    PMB::PrintJob.new(printer_name: printer_name,
                      label_template_id: LabelTemplate.find_by(name: label_template_name).external_id,
  ...
  def tube_layout(printable_object)
    {
      ...
      round_label_bottom_line:       "(#{printable_object[:number]} of #{printable_object[:total_number]}) #{num_prints(printable_object[:num_prints])}"
    }
  end
