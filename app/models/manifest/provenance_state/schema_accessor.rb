...
class Manifest::ProvenanceState::SchemaAccessor < Manifest::ProvenanceState::Accessor
  ...
  def manifest_schema
    @manifest_schema ||= material_schema.dup.tap do |schema|
      ...
      schema['properties'] = config['property_updates'].keys.each_with_object(schema['properties'].dup) do |key, memo|
      ...
      if !labwares.empty? && (labwares[0][:positions])
  ...
  def _build_show_on_form(schema)
    unless schema['show_on_form']
      schema['show_on_form'] = schema['properties'].keys.each_with_object([]) do |k, m|
