...
RSpec.describe SubmissionsController, type: :controller do
  ...
  let(:labware_type) do
    FactoryBot.create :labware_type, num_of_cols: 1,
                                     num_of_rows: 1,
                                     uses_decapper: true
  end
  ...
  describe 'Using the steps defined by wicked' do
    ...
    context 'when user is not authenticated' do
      ...
      it 'redirects to the login page' do
        put :update, params: step_params(manifest, :labware)
    ...
    context 'when user is authenticated' do
      ...
      it 'does not update the manifest if the state is not pending (i.e. broken)' do
        ...
        put :biomaterial_data, params: step_params(manifest, :provenance)
        manifest.reload

        put :update, params: step_params(manifest, :dispatch)
        manifest.reload
      ...
      it 'does not update the manifest state if any required data of steps has not been provided' do
        ...
        put :update, params: step_params(manifest, :dispatch_contact_error)
        manifest.reload
      ...
      it 'does not update manifest status if ProvenanceServive#set_biomaterial_data returns errors' do
        allow_any_instance_of(ProvenanceService).to receive(:set_biomaterial_data).and_return [false, %w[error error]]
      ...
      it 'moves on to the ethics step if the labware contains human material' do
        ...
        put :update, params: step_params(manifest, :labware)
        manifest.reload
        put :biomaterial_data, params: step_params(manifest, :provenance_human)
        manifest.reload
